...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (password_verify($currentPassword, $currentPasswordHash)) {
                ...
                if ($newPassword === $renewPassword) {
                    ...
                    if (mysqli_stmt_prepare($stmtUpdatePassword, $updatePasswordSQL)) {
                        ...
                        mysqli_close($conn);
                        header("Location: user.php?success=password_updated_successfully#change-password");
                        exit();
                    } else {
                    ...
                    mysqli_close($conn);
                    header("Location: user.php?error=new_passwords_do_not_match#change-password");
                    exit();
                }
            } else {
                // Incorrect current password
                mysqli_close($conn);
                header("Location: user.php?error=incorrect_current_password#change-password");
                exit();
            }
    ...
    header("Location: user.php?error=form_not_submitted#change-password");
    exit();
}
?>
