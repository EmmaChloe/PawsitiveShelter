...
<body>
  ...
  <section class="banner">
    ...
    <div class="row">
        ...
        <div class="content">
            <h3>Lost & Found</h3>
        <p>Lost and Found: Reuniting Hearts, One Paw at a Time.</p> 
        </div>
...
<section id="cta" class="cta">
    <div class="container" data-aos="zoom-in">
      ...
      <div class="text-center">
        <h3>Lost Your Pet?</h3>
        <p> Register your lost pet on our Pets Shelter Management System and let us be their voice in reuniting them with their loving family. <br>
            Together, we can bring hope and joy back into their lives.</p>
        <a class="cta-btn" href="lost_detail.php">Register</a>
      </div>
...
<section id="recent-posts" class="recent-posts sections-bg">
    <div class="container" data-aos="fade-up">
        <div class="row gy-4">
            ...
            $pagination_query = "SELECT * FROM LostPet LIMIT $start, $posts_per_page";
            ...
            while ($row = mysqli_fetch_assoc($pagination_result)) {
                ...
                <div class="col-xl-4 col-md-6">
                    <article>
                        <div class="post-img">
                            <img src="images/lost/<?php echo $row['Lost_Photo']; ?>" alt="" class="img-fluid">
                            
                            <?php
                            // Display pet images dynamically
                            $image_folder = "images/lost/";
                        
                            // Check if LostPet_Photo is not empty
                            if (!empty($row['Lost_Photo'])) {
                                // Assume LostPet_Photo is a comma-separated list of image file names
                                $photos = explode(',', $row['Lost_Photo']);

                            } else {
                                // Display a default image if LostPet_Photo is empty
                                echo '<img src="images/icon.png" alt="Default Pet Photo">';
                            }
                            ?>
                        </div>
                        <p class="post-category"><?php echo $row['Lost_Breed']; ?></p>
                        <h2 class="title">
                            <!-- Provide a link to pet_detail.php with the specific Pet_ID -->
                            <a href="pet_detail.php?lost_id=<?php echo $row['Lost_ID']; ?>"><?php echo $row['Lost_Name']; ?></a>
                        </h2>
                        <div class="d-flex align-items-center">
                            <div class="post-meta">
                                <?php
                                // Display additional lostpet information
                                echo '<i class="fas fa-star"><span class="post-author"> ' . $row['Lost_Body'] . ' </span></i><br>';
                                echo '<i class="fas fa-star"><span class="post-author"> ' . $row['Lost_Gender'] . ' </span></i><br>';
                                echo '<p class="post-date"><time datetime="' . $row['Lost_Date'] . '">' . $row['Lost_Date'] . ' | ' . $row['Lost_Location'] . '</time></p>';
                                ?>
                            </div>
                        </div>
                    </article>
        ...
        <div class="row">
            <div class="col-12">
                <ul class="pagination justify-content-center">
                    ...
                    // Previous button
                    if ($page > 1) {
                        echo '<li class="page-item"><a class="page-link" href="lost.php?page=' . ($page - 1) . '">Previous</a></li>';
                    } else {
                    ...
                    for ($i = 1; $i <= $total_pages; $i++) {
                        echo '<li class="page-item ' . ($page == $i ? 'active' : '') . '"><a class="page-link" href="lost.php?page=' . $i . '">' . $i . '</a></li>';
                    }
