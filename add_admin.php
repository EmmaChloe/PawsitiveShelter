...
if (isset($_POST['addadmin'])) {
    ...
    if (empty($name) || empty($email) || empty($phone) || empty($username) || empty($password) || empty($userType)) {
        header("Location: edit_admin.php?error=emptyfields");
        exit();
    } else {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($resultCheck > 0) {
                header("Location: edit_admin.php?error=usernametaken");
                exit();
            } else {
                // Insert user data into the database
                $sql = "INSERT INTO User (User_Name, User_Email, User_Phone, User_Username, User_Password, User_Type) VALUES (?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    echo "SQL Error in INSERT: " . mysqli_stmt_error($stmt);
                    exit();
                } else {
                    mysqli_stmt_bind_param($stmt, "ssssss", $name, $email, $phone, $username, $hashedPassword, $userType);
    ...
    header("Location: edit_admin.php?error=tryagain");
    exit();
}
?>
